...
namespace AstroGrep.AstroGrepBase
{
   ...
   public class Grep
   {
      ...
      public delegate void SearchErrorHandler(FileInfo file, string message);
      /// <summary>A file search threw an error</summary>
      public event SearchErrorHandler SearchError;

      /// <summary>The search has completed</summary>
      /// <param name="message">Description of completion</param>
      public delegate void SearchCompleteHandler(string message);
      /// <summary>The search has completed</summary>
      public event SearchCompleteHandler SearchComplete;

      /// <summary>The search has been cancelled</summary>
      /// <param name="message">Description of cancel</param>
      public delegate void SearchCancelHandler(string message);
      /// <summary>The search has been cancelled</summary>
      ...
      private void StartGrep()
      {
         ...
         catch (ThreadAbortException threadEx)
         {
            ...
            if (SearchCancel != null)
               SearchCancel(string.Format("Grep was aborted. ({0})", threadEx.Message));
         }
         catch (Exception ex)
         {
            //error
            if (SearchError != null)
               SearchError(null, ex.Message);
         }
      }
      ...
      private void Execute(DirectoryInfo SourceDirectory, string SourceDirectoryFilter, string SourceFileFilter, string searchText)
      {
         ...
         foreach ( FileInfo SourceFile in SourceFiles)
         {
            ...
            {
               System.Diagnostics.Debug.WriteLine(threadEx.Message);
               UnloadPlugins();
            }
            catch (Exception ex)
            {
               if (SearchError != null)
                  SearchError(SourceFile, ex.Message);
            }
         }
      }
      ...
      {
         // TLW -- Crashed in Linux because __Plugins was null.
         if (__Plugins != null)
         {
	         for (int i = 0; i < __Plugins.Count; i++)
	         {
