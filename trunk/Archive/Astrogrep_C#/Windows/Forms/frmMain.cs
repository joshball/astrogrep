...
namespace AstroGrep.Windows.Forms
{
   ...
   /// </history>
	public class frmMain : System.Windows.Forms.Form
	{
      ...
      private delegate void UpdateStatusCallBack(System.IO.FileInfo file);
      ...
      private void lstFileNames_ItemDrag(object sender, ItemDragEventArgs e)
      {
         ...
         string path = item.SubItems[Constants.COLUMN_INDEX_DIRECTORY].Text + "\\" + item.SubItems[Constants.COLUMN_INDEX_FILE].Text;
      ...
      private void LoadSettings()
      {
         ...
         if (_state != -1 && _state != (int)FormWindowState.Maximized && _state != (int)FormWindowState.Minimized)
         {
         ...
         else if (_state != -1 && _state == (int)FormWindowState.Maximized)
         {
	...
	public void BugInfo(string message)
	{
      ...
      private void BrowseForFolder()
      {
         ...
         {
            // highlight first entry if available
            if (cboFilePath.Items.Count > 0 && cboFilePath.Items.Count != 1)
               cboFilePath.SelectedIndex = 0;
            else
               cboFilePath.SelectedText = string.Empty;
         }
      }
      ...
      private void ProcessCommandLine()
      {
         ...
         if (args.AnyArguments)
         {
            ...
            chkRegularExpressions.Checked = args.UseRegularExpressions;
            chkCaseSensitive.Checked = args.IsCaseSensitive;
            chkWholeWordOnly.Checked = args.IsWholeWord;
            chkRecurse.Checked = args.UseRecursion;

            chkFileNamesOnly.Checked = args.IsFileNamesOnly;
            chkNegation.Checked = args.IsNegation;
            chkLineNumbers.Checked = args.UseLineNumbers;
      ...
      /// </history>
      private void ReceiveSearchError(System.IO.FileInfo file, string message)
      {
         if (file == null)
            MessageBox.Show(string.Format(Language.GetGenericText("SearchGenericError"), message), Constants.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
         else
            MessageBox.Show(string.Format(Language.GetGenericText("SearchFileError"), file.FullName), Constants.ProductName, MessageBoxButtons.OK, MessageBoxIcon.Error);
      }
      ...
      /// </history>
      private void ReceiveSearchCancel(string message)
      {
         UpdateStatusBar(Language.GetGenericText("SearchCancelled"));
         SetSearchState(true);
      }
      ...
      /// 	[Curtis_Beard]		07/12/2006	Created
      /// </history>
      private void ReceiveSearchComplete(string message)
      {
         UpdateStatusBar(Language.GetGenericText("SearchFinished"));
         SetSearchState(true);
      }
      ...
      /// <summary>
      /// Updates the status bar text with the given file name (thread safe)
      /// </summary>
      /// <param name="file">FileInfo object containg currently being search file</param>
      /// <history>
      ///    [Curtis_Beard]    07/07/2006   Created
      /// </history>
      private void UpdateStatusBar(System.IO.FileInfo file)
      {
         if (stbStatus.InvokeRequired)
         {
            UpdateStatusCallBack _delegate = new UpdateStatusCallBack(UpdateStatusBar);
            stbStatus.Invoke(_delegate, new object[1] {file});
            return;
         }

         stbStatus.Text = string.Format(Language.GetGenericText("SearchSearching"), TruncateFileName(file, stbStatus));
      }
